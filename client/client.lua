...
local store = { 
    [_U('open_store')] = { title="Traficante", size= 10, colour=34, id=274, x = 2560.35, y = 4651.07, z = 33.08}, 
} 

local NPC = {
    {seller = true, model = "a_m_m_ktown_01", x = 2559.72, y = 4649.74, z = 33.08, h = 329.34},
}
...
function OpenArmeroMenu()
    ESX.UI.Menu.CloseAll()

     ESX.UI.Menu.Open(
        'default', GetCurrentResourceName(), 'Armero_markets',
         {
            title    = _U('Armero_market'),
            elements = {
				--{label = ('Kit RCP - <span style="color:green;">8000$</span>'), item = 'medikit', price = 800, amount = 1},
				--{label = ('Vendaje - <span style="color:green;">50$</span>'), item = 'bandage', price = 350, amount = 1},
				{label = ('Cargador -  <span style="color:green;">300$</span>'), item = 'clip', price = 300, amount = 1},
				--{label = ('Antibiótico -  <span style="color:green;">55$</span>'), item = 'antibiotico', price = 55, amount = 1},
            }
        },
        function(data, menu)
            ...
            TriggerServerEvent('esx:Armero', item, price, amount)
        end,
    function(data, menu)
        menu.close()
    end)
